...
PATCHES=(
	...
	"${FILESDIR}"/${PN}-8.0-brotli-unbundle.patch
)
...
src_install() {
	...
	my_doglsym() {
		dosym glxtrace.so "${EPREFIX}"/usr/$(get_libdir)/${PN}/wrappers/libGL.so
		dosym glxtrace.so "${EPREFIX}"/usr/$(get_libdir)/${PN}/wrappers/libGL.so.1
		dosym glxtrace.so "${EPREFIX}"/usr/$(get_libdir)/${PN}/wrappers/libGL.so.1.2
	}

	multilib_parallel_foreach_abi my_doglsym
