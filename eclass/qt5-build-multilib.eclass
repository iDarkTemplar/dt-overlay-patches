...
if [[ (${PN} != qttest && ${PN} != qtwebkit) || (${PN} == qtwebkit && ${QT5_MINOR_VERSION} -lt 9) ]]; then
	DEPEND+=" test? ( ~dev-qt/qttest-${PV}[${MULTILIB_USEDEP}] )"
fi
RDEPEND="
	dev-qt/qtchooser
"
...
multilib_src_configure()	{ qt5_multilib_src_configure; }
multilib_src_compile()		{ qt5_multilib_src_compile; }
multilib_src_test()		{ qt5_multilib_src_test; }
multilib_src_install()		{ qt5_multilib_src_install; }
#multilib_src_install_all()	{ qt5_multilib_src_install_all; }

# @FUNCTION: qt5-build-multilib_src_unpack
# @DESCRIPTION:
# Unpacks the sources.
...
# @FUNCTION: qt5-build-multilib_src_prepare
# @DESCRIPTION:
...
# @FUNCTION: qt5_multilib_src_configure
# @DESCRIPTION:
...
# @FUNCTION: qt5_multilib_src_compile
# @DESCRIPTION:
# Runs emake in the target directories.
qt5_multilib_src_compile() {
	qt5_prepare_env

	qt5_foreach_target_subdir emake
}

# @FUNCTION: qt5_multilib_src_test
# @DESCRIPTION:
# Runs tests in the target directories.
qt5_multilib_src_test() {
	qt5_prepare_env
	...
	local testrunner=${BUILD_DIR}/gentoo-testrunner
	cat > "${testrunner}" <<-_EOF_ || die
	#!/bin/sh
	export LD_LIBRARY_PATH="${BUILD_DIR}/lib:${QT5_LIBDIR}"
	"\$@"
	_EOF_
	chmod +x "${testrunner}"
...
# @FUNCTION: qt5_multilib_src_install
# @DESCRIPTION:
...
# @DESCRIPTION:
# Runs emake install in the target directories.
#qt5_multilib_src_install_all() {
#}

# @FUNCTION: qt5-build-multilib_pkg_postinst
# @DESCRIPTION:
# Regenerate configuration after installation or upgrade/downgrade.
qt5-build-multilib_pkg_postinst() {
	multilib_foreach_abi qt5_regenerate_global_configs
}
...
qt_use_compile_test() {
	...
	if ! use "$1"; then
		...
		echo "CONFIG += done_config_${2:-$1}" >> "${BUILD_DIR}"/.qmake.cache || die
	fi
}
...
qt5_prepare_env() {
	...
	if [[ ${QT5_MODULE} == qtbase ]]; then
		...
		export QMAKEMODULES="${BUILD_DIR}/mkspecs/modules:${S}/mkspecs/modules:${QT5_ARCHDATADIR}/mkspecs/modules"
	fi
}
...
qt5_foreach_target_subdir() {
	...
	multilib_is_native_abi && [[ -n ${QT5_EXAMPLES_SUBDIRS[@]} ]] && QT5_FOREACH_DIRS+=("${QT5_EXAMPLES_SUBDIRS[@]}")
	...
	for subdir in "${QT5_FOREACH_DIRS[@]}"; do
		...
		mkdir -p "${BUILD_DIR}/${subdir}" || die -n || return $?
		pushd "${BUILD_DIR}/${subdir}" >/dev/null || die -n || return $?
